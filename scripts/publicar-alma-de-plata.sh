#!/bin/bash
# PUBLICADOR CAMPAÑA ALMA DE PLATA - CRÁNEO NOBLE
# Funciona con curl (no requiere Node.js, Python ni Docker)
# Se ejecuta desde QNAP (n8n/cron), GitHub Actions, o cualquier servidor

cd "$(dirname "$0")/.." || exit 1
LOG="cron-alma-de-plata.log"
echo "[$(date)] Iniciando..." >> "$LOG"

# Leer día actual del estado
DIA=$(python3 -c "import json; print(json.load(open('.campana-alma-de-plata-state.json'))['currentDay'])" 2>/dev/null)
...
# Buscar imagen en contenido/alma-de-plata/dia{N}/
IMGDIR="contenido/alma-de-plata/dia${DIA}"
IMAGEN=$(ls "$IMGDIR" 2>/dev/null | grep -iE '\.(png|jpg|jpeg|webp)$' | head -1)

# Extraer copy del markdown (formato: ## Día N — Título\nTexto...)
python3 -c "
import re, sys
with open('marketing/campana-alma-de-plata.md') as f:
    content = f.read()
marker = f'## Día $DIA —'
start = content.find(marker)
if start == -1: sys.exit(1)
section = content[start:]
# Find next section (## Día or ## Respuestas or end)
end = len(section)
for pat in [f'## Día ', '## Respuestas']:
    pos = section.find(pat, 10)
    if pos != -1 and pos < end: end = pos
lines = section[:end].split('\n', 1)
text = lines[1].strip() if len(lines) > 1 else ''
if text:
    with open('/tmp/_copia_alma.txt', 'w') as out:
        out.write(text)
else:
    sys.exit(1)
" 2>/dev/null || { echo "[$(date)] ERROR: No se pudo extraer copy dia $DIA" >> "$LOG"; exit 1; }

COPY=$(cat /tmp/_copia_alma.txt)
...
if [ "$FB_ID" != "ERROR" ]; then
    ...
    python3 -c "
import json
with open('.campana-alma-de-plata-state.json') as f:
    s = json.load(f)
s['publishedDays'].append(s['currentDay'])
s['currentDay'] += 1
with open('.campana-alma-de-plata-state.json', 'w') as f:
...
"
    ...
    if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_CHAT_ID" ]; then
        curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
            ...
            -d "text=✅ Alma de Plata: Día $DIA publicado ($FB_ID)" > /dev/null 2>&1
    fi
else
